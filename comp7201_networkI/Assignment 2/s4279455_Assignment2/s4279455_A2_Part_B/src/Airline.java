...
*/
final public class Airline extends UDPCommTravelAgency {
	...
	public Airline(int nameServerPort, String nameServerIp, String airline) throws Exception {
		...
		registerNameServer("register:Airline"+airline+":"+listeningPort, this.nameServerIp, this.nameServerPort);
	...
	protected void takeAction(String line) throws Exception {
		if (reservationIp == null) {
		String[] temp1 = lookup("ReservationService:Airline"+airline, nameServerIp, nameServerPort);
		reservationIp = temp1[0];
		reservationPort = Integer.parseInt(temp1[1]);
		}
		if (bankIp == null) {
			String[] temp2 = lookup("Bank:Airline"+airline, nameServerIp, nameServerPort);
			bankIp = temp2[0]; 
		...
		if (tokens.length == 4 && tokens[0].compareTo("enquiry")== 0) {
				...
				sendUDP(replyMessage, reservationIp, reservationPort);			
		//Receiving Purchase air ticket request: From ReservationServer : Message -> requestPurchase: automatic|manual : VA|QF : flight_number : origin_airport : destination_airport : customer_name: credit_card
				...
				if (flightDetail == null) { 
					sendUDP("responsePurchase:"+tokens[1]+":Error:Cannot find any flights between the origin and destination airport", reservationIp, reservationPort);
					return;
				}
				Flight f = null;
				try 
				{
					...
					if (f == null) {
						sendUDP("responsePurchase:"+tokens[1]+":Error:Could not find flight matching flight number", reservationIp, reservationPort);
						return;
					}
				} catch (Exception ex) {
					sendUDP("responsePurchase:"+tokens[1]+":Error:Could not find flight matching flight number", reservationIp, reservationPort);
				...
				if (f.seat == 0) {
					...
					sendUDP("responsePurchase:"+tokens[1]+":Error:There are no more seats left on flight "+tokens[2], reservationIp, reservationPort);
					return;									
				} else {
					f.seat -= 1;
					...
					sendUDP("Airline"+this.airline+":"+line, bankIp, bankPort);
				}
		// Receiving Response of Purchase : From Bank : Message -> responsePurchase: automatic|manual : Ok|Error : Message
			...
			sendUDP(line, reservationIp, reservationPort);			
		}
	}
